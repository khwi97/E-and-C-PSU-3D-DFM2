...
def fine_tune_siamese_network(
    ...
    if thick_weight is not None:
        model_dtype = next(model.parameters()).dtype
        weight_tensor = thick_weight.to(device=device, dtype=model_dtype).requires_grad_(False)
    ...
    for epoch in range(epochs):
        ...
        for batch in tqdm(trainLoader, desc=f"Fine-tuning Epoch {epoch+1}/{epochs}"):
            ...
            if thick_weight is not None:
                img1_weighted = img1 * weight_tensor
                img2_weighted = img2 * weight_tensor
            else:
                img1_weighted = img1
                img2_weighted = img2
            
            optimizer.zero_grad()
            outputs = model(img1_weighted, img2_weighted)
        ...
        with torch.no_grad():
            for v_img1, v_img2, v_labels in valLoader:
                ...
                if thick_weight is not None:
                    v_img1_weighted = v_img1 * weight_tensor
                    v_img2_weighted = v_img2 * weight_tensor
                else:
                    v_img1_weighted = v_img1
                    v_img2_weighted = v_img2

                v_out = model(v_img1_weighted, v_img2_weighted)
...
def recompute_accept_probs_from_cached_imgs(
    ...
    with torch.no_grad():
        for s in range(0, N, batch_size):
            ...
            weight_tensor = thick_weight.to(device=device, dtype=model_dtype)
            img_weighted = img * weight_tensor
            tgt_weighted = tgt * weight_tensor
            
            logits = model(img_weighted, tgt_weighted)
...
def main():
    ...
    N_accept = 1000
    New_Start = True
    save_dir = '/home/calibration_result/mainCBAMConcat7_obs_result'
    checkpoint_directory = '/home/calibration_result/mainCBAMConcat7_obs_training'
    ...
    target_idx = 400
    target = torch.load('/home/newdata/thick_obs.pt', map_location="cpu")
    target = target / maximum
    thick_weight = torch.load('/home/newdata/discrepancy_weight.pt', map_location="cpu")
    thick_weight = thick_weight.to(device)
    ...
    print(f"Start ABC: Target is observation. Core Buffer Size: {core_real_images.size(0)}")
    ...
    save_path_core_emul = os.path.join(save_dir, f"core_emul_images_tgt_obs.pt")
    if os.path.exists(save_path_core_emul):
        print(f"Loading cached core emulator images for obs...")
        core_emul_images = torch.load(save_path_core_emul, map_location="cpu")
    else:
        print(f"Generating emulator images for {core_real_images.size(0)} parameters (Target obs removed)...")
        ...
        if os.path.exists(saved_param_path):
            ...
            with torch.no_grad():
                while accepted_count < N_accept:
                    ...
                    # Add to weighted version for acceptance probability calculation
                    weight_tensor = thick_weight.to(device=device, dtype=model_dtype)
                    img_gen_weighted = img_gen * weight_tensor
                    target_batch_weighted = target_batch * weight_tensor
                    
                    logits = model(img_gen_weighted, target_batch_weighted).squeeze()
